...
public class AbpTemplateModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        {
            options.IsJobExecutionEnabled = true; //enables job execution
        });

        //added
        Configure<AbpDistributedCacheOptions>(options =>
        {
            options.KeyPrefix = "MyApp:"; // Optional, adds a prefix to all cache keys
            options.GlobalCacheEntryOptions = new Microsoft.Extensions.Caching.Distributed.DistributedCacheEntryOptions
            {
                AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(20) // Default cache expiration
            };
        });
        ...
        {
            options.Configuration = "127.0.0.1:6379"; // Your Redis server address
                                                      //options.InstanceName = "MyAppInstance";   // Optional, adds a prefix to keys
        });
    ...
    {
        context.Services.AddAutoMapperObjectMapper<AbpTemplateModule>();
